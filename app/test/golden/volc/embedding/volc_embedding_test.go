// Package embedding runs golden conformance cases for the Volc Ark
// embedding adapter. Mirrors the deepseek harness structure verbatim
// — once a third provider gains coverage we'll lift the harness out
// into test/golden/framework alongside RunChat.
package embedding
...
import (
	...
	volcEmb "github.com/llmhub/llmhub/internal/provider/volc/embedding"
)
...
func TestGoldenVolcEmbedding(t *testing.T) {
	cases, err := loadCases(".")
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	...
	for _, c := range cases {
		...
		t.Run(c.Name, func(t *testing.T) {
			...
			adapter := volcEmb.New(provider.Config{
				ID:      "volc",
				BaseURL: ts.URL,
